    ...
    <Layout className="admin-layout">
      <Layout.Aside className="sidebar" width="248px">
        ...
        <nav>
          <button
            type="button"
            className={location.pathname.startsWith('/threads') ? 'nav-item active' : 'nav-item'}
        ...
        <Button variant="text" className="logout" onClick={() => void onLogout()}>
          <LogoutIcon /> 退出登录
        </Button>
      </Layout.Aside>
...
function ThreadsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(errorText(cause, '讨论列表加载失败'));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        action={
          <>
            <Button variant="outline" loading={refreshing} onClick={() => void refreshMetadata()}>
              <RefreshIcon /> 刷新全部元数据
            </Button>
            ...
            {visible.length ? (
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {visible.map((thread) => (
                      <tr key={thread.id} onClick={() => navigate(`/threads/${thread.id}`)}>
                        <td>
              ...
              !loading && (
                <Empty
                  title={query ? '没有匹配的讨论串' : '还没有讨论串'}
                  description={query ? '请尝试其他关键词。' : '收到第一条评论后会显示在这里。'}
                />
              )
            )}
          </Loading>
...
function orderTree(posts: Post[], ascending: boolean): Array<{ post: Post; depth: number }> {
  ...
  const visit = (post: Post, depth: number) => {
    ...
    const replies = [...(children.get(post.id) ?? [])].sort((a, b) =>
...
function ThreadPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(errorText(cause, '讨论加载失败'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function toggleHidden(post: Post, hidden: boolean) {
    try {
      ...
      replacePost(updated);
      setThread((current) =>
        current ? { ...current, amount: Math.max(0, current.amount + (hidden ? -1 : 1)) } : current,
      );
      void MessagePlugin.success(hidden ? '评论已隐藏' : '评论已恢复显示');
    } catch (cause) {
      void MessagePlugin.error(errorText(cause, '操作失败'));
    }
  }
  ...
  async function createPost(parent: number, content: string) {
    ...
    try {
      ...
      void MessagePlugin.error(errorText(cause, '发布失败'));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function copyChain(post: Post) {
    ...
    try {
      ...
      void MessagePlugin.success(`已复制 ${chain.length} 层评论链`);
    } catch {
  ...
  if (!validThreadId)
    return (
      <div className="page">
        <Button variant="text" className="back-button" onClick={() => navigate('/threads')}>
    ...
    <div className="page">
      ...
      <PageHeader
        ...
        action={
          thread && (
            <>
              <Button variant="outline" onClick={() => navigate(`/threads/${threadId}/edit`)}>
                <EditIcon /> 编辑元数据
              </Button>
              <Button
                theme="primary"
          ...
          {thread && (
            <>
              <div className="thread-strip">
                ...
                <Button size="small" variant="text" onClick={() => void load()}>
                  <RefreshIcon /> 刷新
                </Button>
              </div>
              <Card className="admin-compose" bordered={false}>
                ...
                <Button
                  ...
                  onClick={() => void createPost(0, topContent)}
                >
                  发布评论
                </Button>
              </Card>
                    ...
                    <div className="comment-actions">
                      <Space breakLine>
                        ...
                        <Button
                          size="small"
                          variant="text"
                          onClick={() => navigate(`/threads/${threadId}/posts/${post.id}/edit`)}
                        >
                          <EditIcon /> 完整编辑
                        </Button>
                        {post.parent > 0 && (
                          <Button size="small" variant="text" onClick={() => jumpToParent(post.parent)}>
                            <JumpIcon /> 跳到父评论
                          </Button>
                        )}
                        <Button size="small" variant="text" onClick={() => void copyChain(post)}>
                          <CopyIcon /> 复制祖先链
                        </Button>
                      </Space>
                      <label>
                        ...
                        <Switch
                          size="small"
                          value={!post.hidden}
                          onChange={(visible) => void toggleHidden(post, !visible)}
                        />
                      </label>
                    </div>
                  </article>
      ...
      <Dialog
        ...
        onConfirm={() => replying && void createPost(replying.id, replyContent)}
...
function PostEditPage() {
  ...
  async function load() {
    ...
    setError('');
    try {
      setPost(await api.getPost(threadId, targetId));
    } catch (cause) {
      setError(errorText(cause, '评论加载失败'));
    } finally {
      setLoading(false);
    }
  }
  useEffect(() => {
    void load();
  }, [threadId, targetId, valid]);

  async function save() {
    if (!post || !post.name.trim() || !post.email.trim() || !post.content.trim())
      return void MessagePlugin.warning('姓名、邮箱和内容不能为空');
    setSaving(true);
    try {
      await api.updatePost(threadId, post);
      void MessagePlugin.success('评论已更新');
      navigate(`/threads/${threadId}`);
    } catch (cause) {
      void MessagePlugin.error(errorText(cause, '保存失败'));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="page narrow-page">
      <Button variant="text" className="back-button" onClick={() => navigate(`/threads/${threadId}`)}>
        <ChevronLeftIcon /> 返回讨论详情
      </Button>
      <PageHeader
        eyebrow={`POST / ${targetId}`}
        title="完整编辑评论"
                ...
                <Form layout="vertical">
                  ...
                  <div className="toggle-grid">
                    ...
                    <label>
                      接收邮件{' '}
                      <Switch
                        value={post.receiveEmail}
                        onChange={(receiveEmail) => setPost({ ...post, receiveEmail })}
                      />
                    </label>
                    <label>
                  ...
                  <div className="form-actions">
                    <Button variant="outline" onClick={() => navigate(`/threads/${threadId}`)}>
                      取消
                    </Button>
...
function ThreadEditPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(errorText(cause, '元数据加载失败'));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="page narrow-page">
      ...
      <PageHeader eyebrow={`THREAD / ${threadId}`} title="编辑讨论元数据" detail="评论统计和活动时间由服务端维护。" />
      {error ? (
        <ErrorState message={error} retry={() => void load()} />
      ) : (
        <Loading loading={loading} showOverlay>
    ...
    return (
      <main className="auth-state">
        <Card bordered={false}>
          ...
          <Button theme="primary" onClick={() => void checkSession()}>
            重试
          </Button>
        </Card>
      </main>
